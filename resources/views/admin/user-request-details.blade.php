<x-app-layout>
    ...
    <div class="request-details p-5 rounded-lg bg-white">
        ...
        <div class="request-header flex items-center justify-between w-full mb-3">
            <div class="mt-4">
                ...
                @php
                    ...
                    $statusColors = config('status'); // get the full status config
                    $colors = $statusColors[$status] ?? ['text' => 'text-gray-700', 'bg' => 'bg-gray-200'];
                @endphp



                <span class="px-4 py-1 rounded-full text-sm font-semibold {{ $colors['text'] }} {{ $colors['bg'] }}">
                    {{ $request->computed_status }}
                </span>
            </div>
                ...
                @auth
                    @if(auth()->user()->role === 'admin' && $request->status === 'Open')
                        <form action="{{ route('admin.requests.accept', $request->id) }}" method="POST">
                            @csrf
                            <x-primary-button type="submit" style="background-color:#22C55E;color:white;width:100px;height:40px;">
                                Accept
                            </x-primary-button>
                        </form>
                        ...
                        <form action="{{ route('admin.requests.decline', $request->id) }}" method="POST">
                            @csrf
                            <x-primary-button type="submit" style="background-color:#EF4444;color:white;width:100px;height:40px;">
                                Decline
                            </x-primary-button>
                        </form>
                    @elseif($request->status === 'Open')
                        <div x-data="{ openCancelForm: false }">
                            <x-primary-button @click="openCancelForm = true"
                        ...
                        <div x-data="{ openEdit: false }">
                            <x-primary-button @click="openEdit = true"
                                style="background-color:#0575E6;color:white;width:100px;height:40px;">
                                Edit
                            </x-primary-button>
                ...
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="detail-text">
                            ...
                            {{ \Carbon\Carbon::parse($request->created_at)->format('d, Y') }}
                        </p>
                    </div>
                </div>
        ...
        <div class="request-information flex justify-between pt-3">
            ...
            <p class="detail-text">
                {{ $request->handledByAdmin ? $request->handledByAdmin->name : '-' }}
            </p>
...
<style>
    ...
    .left-col-info, .right-col-info { width:48%; min-width:250px; }
