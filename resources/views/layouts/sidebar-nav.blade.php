@php
    ...
    $isFirstAdmin = $isAdmin && $user->id === $firstAdminId;
@endphp
...
<aside class="sidebar flex flex-col h-screen relative
    ...
    <ul class="sidebar-links">
        ...
        @if($isAdmin || $isPersonnel)
            ...
            {{-- CREATED ADMINS (FIRST ADMIN ONLY) --}}
            @if($isFirstAdmin)
            <li>
            ...
            <li>
                <a href="{{ route('admin.personnel') }}"
                   class="flex items-center gap-2
                          text-gray-700 dark:text-gray-300
                          {{ request()->routeIs('admin.personnel*') ? 'active text-blue-600 dark:text-blue-400' : '' }}">
